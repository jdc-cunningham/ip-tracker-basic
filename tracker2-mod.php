...
function track2() {
	...
	// create function to insert an entry in visits table
	function insertVisitsEntry($link, $id, $client_ip, $url_requested, $date_requested) {
	...
	if($client_count > 0) {
	    ...
	    $stmt2->execute();
	    // Insert visits entry
	    insertVisitsEntry($link, $id, $client_ip, $url_requested, $date_requested);
	}
	else {
